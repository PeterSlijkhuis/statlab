...
<CodeBlock id="skew" code={`library(dplyr)
library(ggplot2)
...
population %>%
  ...
  labs(title = "Stress in the population", x = "Stress score", y = "Frequency") +
  theme_classic()`} />
...
tibble(mean_stress = means_40) %>%
  ggplot(aes(x = mean_stress)) +
  geom_histogram(bins = 40) +
  labs(title = "Means of 40 students", x = "Sample mean stress", y = "Frequency") +
  theme_classic()`} />
