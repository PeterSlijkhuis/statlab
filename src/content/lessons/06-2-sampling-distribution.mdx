...
times. `replicate()` does the repeating. This is one of the few places we use
base R: `mean(sample(population$stress, 10))` works on a single column and is
cheap to repeat, while a dplyr pipeline over the whole data frame would do far
more work each of the 1000 times.
...
<Predict
  ...
  choices={[
    ...
    { text: 'The sample means', response: 'Look again at how wide each histogram is along the shared stress axis. Means cluster far more tightly than individuals do.' },
    { text: 'They are equally spread', response: 'Compare how much of the shared stress axis each histogram covers. The means occupy a much narrower interval.' },
  ]}
/>
